...
document.getElementById('btnInformer').addEventListener('click', (e) => {
	...
	if (numTabActive == 2) {
		...
		alerter(
			...
			`<Text trans="text+:sendCodeError1444444444;">Вы можете скопировать в память различную информацию по своему времени и переслать её пользователям различными способами.</Text><br><br><p>Информация по занятому времени пользователя. <br></p><a href="${txtLink}"><p style="font-size:0.90em"><b>${txtLink}</b></p></a><p>Приглашение пользователю заполнить свое свободное время <br><a href="${txtInvite}"><p style="font-size:0.90em"><b>${txtInvite}</b></p></a></p><p>Ссылка с текстом busy</p>`,
			'standart',
			'info',
			'standart'
		);
		...
		alerter(
			'<Text trans="text+:AlertErrorHeader4444444;">Информеры</Text>',
			`<Text trans="text+:sendCodeError1444444444;">Вы можете скопировать в память различную информацию по своему времени и переслать её пользователям различными способами.</Text><br><p>Информация по cвободному времени пользователя. <br></p><a href="${txtLink}"><p style="font-size:0.90em"><b>${txtLink}</b></p></a><p>Приглашение пользователю заполнить свое свободное время <br><a href="${txtInvite}"><p style="font-size:0.90em"><b>${txtInvite}</b></p></a></p><p>Ссылка с текстом free <button type="button" class="btn btn-primary ml-3 mr-1 btn btn-sm" onclick="let a = 'данные в буфере';navigator.clipboard.writeText(text).then(() => {});alert(a);">копировать</button></p>`,
			'standart',
			'info',
			'standart'
		);
