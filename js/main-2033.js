...
function setItems(currentUser, user = true) {
 ...
 <Text trans='text+:cntQuery;'></Text><b>${usersToDisplay.length}</b><br>                                    <Text trans='text+:maxNOM;'></Text><b>${findBestMatch()[0][0]}
(${Math.round(findBestMatch()[0][0] * 100/usersToDisplay.length)}%)
</b><br><div style="margin-top:7px;margin-bottom:7px;"><b><Text trans='text+:d1;'>Данные по выбранному времени</Text></b></div><Text trans='text+:MatchTimeText;'><br></Text>                                      <b style="color:#48c2a9; margin-bottom:7px;">${temp.slice(1).join(", ")}</b><br>                            <div style="margin-top:7px; margin-bottom:7px"><Text trans='text+:llll;'></Text><b>${curDay[j].getAttribute("maximum") === "1"? '<Text trans="text+:yes;"></Text>': '<Text trans="text+:no;"></Text>'}</b></div><Text trans='text+:NotMatched;'></Text><b style="color:indianred;">${usersToDisplay.filter(user => !ids.includes(user.id)).map(user => user.name).join(", ")}</b></div>`);
...
function loadStudentsTable() {
	...
	console.log(filteredUsers, "514 - loadStudentsTable()");
	console.log(sessionStorage.getItem("typeBase"), sessionStorage.getItem("globalLogin"), "514")
...
<li>Отправить клиенту приглашение отметить своё свободное или занятое время и затем быстро и просто ввести его данные в систему для составления расписания</li><li>Отправить клиентам своё свободное или занятое время</li><li>Предоставить доступ к расписанию нескольким сотрудникам</li><li>Управлять персональным расписанием сотрудника администратором времени (тайм-менеджером) </li></ul></Text> `,
					"standart",
					"success",
					'<a href="https://settime.online"><button type="button" class="btn btn-sm btnUpdate m-2 " style="font-weight: 500;color: black; font-size: 1em; background-color:#48c2a9;">Понятно!</button></a>', true)
	...
	try {
		...
		const bottomtable = document.createElement("p");
...
//			if (i == sortedFilteredUsers.length - 1) {
	...
	//				tableFirstTdB.setAttribute("colspan", "5")
	//				tableFirstTdB.style.textAlign = "left"
	//				tableFirstTdB.style.fontWeight = "600"
	//				tableFirstTdB.style.backgroundColor = "white";
	//				tableFirstTdB.textContent = ` Итого: ` + String(sortedFilteredUsers.length) + " человек(а)";
	//
	//
...
function tempUserUpdate(user) {
	...
	saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'users', JSON.stringify(users))
...
function saveBusySchedule() {
	...
	if (fullName.value !== "") {
		...
		saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'business', JSON.stringify(business))
...
function splittingCourses() {
	...
	saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'courses', JSON.stringify([...courses, ...options]))
	//	localStorage.setItem("courses", JSON.stringify([...courses, ...options]));

	saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'filteredCourses', JSON.stringify([...filteredCourses, ...options]))
...
function splittingGrades() {
	...
	saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'grades', JSON.stringify([...grades, ...options]))
	//	localStorage.setItem("grades", JSON.stringify([...grades, ...options]));

	saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'filteredGrades', JSON.stringify([...filteredGrades, ...options]))
	//	localStorage.setItem(
	//		"filteredGrades",
...
export function filterUsers() {
	...
	saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'filteredUsers', JSON.stringify(filteredUsers))

	//	alert(filteredUsers, '2333')
	console.log(filteredUsers, '2357 - filterUsers()')
	console.log(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), '2357')
	console.log(sessionStorage.getItem('users'), sessionStorage.getItem('filteredUsers'), '2357')
	console.log('-----------')
	//	alert("stop filterUser")
...
function createModal() {
	if (!body.contains(document.querySelector(".filter-modal"))) {
		...
		if (window.innerWidth < 400) {
			...
			filtersDiv.style.paddingTop = "155px";
		} else {
		...
		applyFilter.addEventListener("click", () => {
			...
			saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'filteredCourses', JSON.stringify(filteredCourses.filter(item => item !== "All")))
			...
			saveDataTo(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'filteredGrades', JSON.stringify(filteredGrades.filter(item => item !== "All")))
	...
	for (let i = 0; i < allCourses.length; i++) {
		...
		label.innerHTML = allCourses[i].padEnd(20, " ");
...
import2.addEventListener('click', () => {
	...
	localStorage.setItem('typeBase', "local")
	alert(
		'Глобальный логин: ' + sessionStorage.getItem('globalLogin') + ' Доступ в систему: ' + sessionStorage.getItem('globalAccess')
	);
...
export function start() {
	...
	//	if ((localStorage.getItem('typeBase') == 'remote') && (!sessionStorage.getItem("typeBase"))) {
	//		localStorage.setItem('typeBase', 'local')
	//		sessionStorage.setItem('typeBase', 'local')
	//	}
	...
	console.log(filteredUsers, '3328 - start() ')
	console.log(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), '3328')
	console.log(sessionStorage.getItem('users'), sessionStorage.getItem('filteredUsers'), '3328')
	console.log('-----------')
...
//	//    const users1 = JSON.parse(getDataFrom(localStorage.getItem('typeBase'), globalLogin, 'users')) || [];
//
//	const users = JSON.parse(getDataFrom(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), 'users')) || [];
//
//	//    const filteredUsers =
//	//        JSON.parse(localStorage.getItem("filteredUsers")) || users;
//
...
//	//
//	//	alert(filteredUsers, '523')
//	console.log(filteredUsers, '501 - loadStudentsTable()')
//	console.log(sessionStorage.getItem('typeBase'), sessionStorage.getItem('globalLogin'), '501')
//	console.log(sessionStorage.getItem('users'), sessionStorage.getItem('filteredUsers'), '501')
//	console.log('-----------')
//
//	let rightToolBar = document.createElement("span");
//
