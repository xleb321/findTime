...
function loadStudentsTable() {
	...
	if (!document.getElementById('plus-button')) {
		...
		plusButton.style.fontSize = '1.5em';
...
function loadBusinessTable() {
	...
	if (businessTableDiv.children.length === 0) {
		...
		if (!document.getElementById('plus-button-Busy1')) {
			...
			plusButtonIconBusy.setAttribute('class', 'bi bi-plus');
			plusButtonIconBusy.style.color = 'black';
			plusButtonIconBusy.style.fontSize = '1.5em';
			plusButtonIconBusy.style.fontWeight = 'bold';
...
window.onload = () => {
	...
	if (url.searchParams.get('action')) {
		$('#staticBackdrop').modal('show');
		if (
			url.searchParams.get('action') == 'invite' ||
			url.searchParams.get('action') == 'newUserInvite'
		) {
			...
			document
				...
				.setAttribute('trans', 'text+:AddTime;');
			document.getElementById('loginSendTimeCode').value =
				url.searchParams.get('loginUser');

			alerter(
				'<Text trans="text+:Welcome;">Добро пожаловать!</Text>',
				'<Text trans="text+:InviteText;">Вы открыли приглашение заполнить данные по Вашему свободному или занятому времени.  Введите пожалуйста свое имя и фамилию, отметьте в таблице время и нажмите кнопку отправить! Спасибо!</Text> ',
				'standart',
				'success',
				'standart'
			);
			...
			//            alert(document.getElementById("timeSelect").value)
			//            alert(url.searchParams.get('typeTime'))
...
function F4Team() {
	...
	let bodyWin = /*html*/ `
        <div class="pl-0" style="text-indent:0px;"> 
            ...
            <br>
            <b>Гетман Данил</b> - PhPDev (verification)
            <br>
        </div>
    `;
