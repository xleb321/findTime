...
let btnSaveTableScheduleFull = document.getElementById("btnSaveTableScheduleFull");
...
let btnLoadTableScheduleFull = document.getElementById("btnLoadTableScheduleFull");
...
function alertEmptyStart() {
    ...
    function alertEmpty() {
        ...
        if (checkFlagTime.length == 0) {
            alerter('<Text name="Danger">Внимание!</Text>', '<Text name="AlertEmpty">Отметьте сначала записи <img src="/img/check-square-fill.svg"> во вкладках <b>"Cвободное время"</b> и(или) <b>"Занятое время"</b> и затем вернитесь на вкладку <b>"Расписание"</b>. На ней будет отмечено совпадения по времени.<p style="text-align: justify;">Перед началом работы, пожалуйста, ознакомьтесь c <a href="guide.html"> <b>"Руководством пользователя".</b></a></Text>', 'standart', 'danger', 'standart')
        }
    }
}
...
document.getElementById("logout").addEventListener('click', () => {
    ...
    document.getElementById("btnModalEnterIcon").classList.replace('bi-person-check', 'bi-box-arrow-in-right');
...
if (globalAccess == 1) {
    document.getElementById("btnModalEnterIcon").classList.replace('bi-box-arrow-in-right', 'bi-person-check');
...
function genContentWinEnter(text) {
    ...
    if (Number(objectPHP["access"]) == 0) {
        ...
        if (objectPHP["answer"] == "noUser") {
            contentWinEnter += 'Доступ  в систему: <b style="color:indianred;">НЕ ВЫПОЛНЕН!</b><br>';
            contentWinEnter += objectPHP["txtMsg"];
        } else if (objectPHP["answer"] == "presentUser") {
            contentWinEnter += 'Регистрация в системе: <b style="color:indianred;">НЕ ВЫПОЛНЕНА!</b><br>';
            contentWinEnter += objectPHP["txtMsg"]

        } else {
            contentWinEnter += 'Доступ  в систему: <b style="color:indianred;">НЕ ВЫПОЛНЕН!</b>';
        }
    ...
    // ДЕМО ДОСТУК К СИСТЕМЕ НОВЫХ ПОЛЬЗОВАТЕЛЕЙ НА 30 ДНЕЙ!
        ...
        contentWinEnter += '<p style="font-weight:400;color="skyblue">' + objectPHP["txtMsg"] + '<br></p><br>';
        contentWinEnter += 'Доступ в систему: <b style="color:#48c2a0;">ВЫПОЛНЕН В ДЕМО-РЕЖИМЕ!</b><br>';

        document.getElementById("btnModalEnterIcon").classList.replace('bi-box-arrow-in-right', 'bi-person-exclamation');
        document.getElementById("btnModalEnterIcon").style.color = "IndianRed";
        contentWinEnter += 'Дата окончание доступа: <b style="color:indianred;">' + objectPHP["expiredDate"] + "</b><br>";
        ...
        contentWinEnter += 'Доступ в систему: <b style="color:#48c2a0;">ВЫПОЛНЕН!</b><br>';
        contentWinEnter += 'Дата окончание доступа: <b  style="color:#48c2a0;">' + objectPHP["expiredDate"] + "</b><br>";
        document.getElementById("btnModalEnterIcon").classList.replace('bi-box-arrow-in-right', 'bi-person-check');
        document.getElementById("btnModalEnterIcon").style.color = "#48c2a9";
...
function loadFiles(globalLogin, item) {
    let dataItem = 'https://findtime.online/php/' + globalLogin + '/' + globalLogin + '_' + item + '.json';
...
if (localStorage.getItem('typeBase') && globalLogin != "" && globalAccess != 0) {
...
function radioSelect() {
    ...
    alerter('<Text name="AlertChangeStorageHeader">Смена типа хранилища</Text>', '<Text name="AlertChangeStorageBody">Тип базы данных будет изменён после перезагрузки.</Text>', 'standart', 'warning', 'standart')
    location.reload();
}
...
document.getElementById("sendCode").addEventListener('click', () => {
    ...
    let loginSendTimeCode = document.getElementById("sendCode").value

    let searchParams = new URLSearchParams();
    searchParams.set('login', globalLogin);
    searchParams.set('email', input1Pass.value);
    searchParams.set('action', action);
...
//    alerter('<Text name="info">Информация!</Text>', '<Text name="SendCode">Данные кода времни отправлены на email пользователя системы.</Text>', 'standart', 'info', 'standart')
