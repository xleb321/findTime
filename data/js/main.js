                        ...
                        if (curDay[j].style.background === "transparent") {
                            ...
                            if (curDay[j].children.length === 0) {
                                ...
                                curDay[j].addEventListener("click", () => {
                                    ...
                                    alerter(
                                        "<Text name='MatchTime'>Is a match in time</Text>",
                                        `<Text name='MatchTimeText'>Names of those who matched at this time:<br><b></Text> ${temp
											.slice(1)
											.join(", ")}</b>`
                                    );
                                });
...
function loadStudentsTable() {
    ...
    if (!document.getElementById("help-button")) {
        ...
        helpButton.setAttribute("name", "PlusButton");
        helpButton.setAttribute("id", "create");
        helpButton.setAttribute("id", "help-button");
        helpButton.setAttribute("class", "help");
        helpButton.setAttribute("style", "color: black; background-color:deepskyblue;box-sizing: border-box; padding: 0px 3px 2px 4px; border-radius: 2px; border-width: 0px; font-size: 1.05em; font-weight: 100;");
    ...
    if (
        ...
        text.setAttribute("name", "EmptinessOfUsersList");
    ...
    if (!document.getElementById("section")) {
        ...
        text.setAttribute("name", "SortingLabel");
        ...
        textElement.setAttribute("name", "NameGradeCourse");
...
function loadBusinessTable() {
    ...
    if (business.length === 0) {
        ...
        text.setAttribute("name", "EmptinessOfBusinessList");
        ...
        if (!document.getElementById("help-button-busy")) {
            ...
            let helpButtonIcon = document.createElement("i");
            //               alert("help1busy")
            helpButtonBusy.setAttribute("name", "PlusButton");
            ...
            helpButtonBusy.setAttribute("style", "color: black; background-color:deepskyblue;box-sizing: border-box; padding: 0px 3px 2px 4px; border-radius: 2px; border-width: 0px; font-size: 1.05em; font-weight: 100;");
    ...
    if (businessTableDiv.children.length === 0) {
        ...
        nameAscendingOption.setAttribute("name", "DescAscending");
        nameDescendingOption.value = "nameDescending";
        nameDescendingOption.innerHTML = "&#8593; description desc";
        nameDescendingOption.setAttribute("name", "DescDescending");
        numAscendingOption.value = "numAscending";
        numAscendingOption.innerHTML = "&#8595; num asc";
        numAscendingOption.setAttribute("name", "NumAscending");
        numDescendingOption.value = "numDescending";
...
function saveUser() {
    ...
    if (currentCourse && currentLevel && fullName.value !== "") {
        ...
        alerter('<Text name="AlertErrorHeader">Ошибка</Text>', '<Text name="AlertErrorBody1">Информация введена не полностью. Введите курс, уровень и имя</Text>', 'standart', 'danger', 'standart');
    }
}
...
function saveBusySchedule() {
    ...
    if (fullName.value !== "") {
        ...
        alerter('<Text name="AlertErrorHeader">Ошибка</Text>', '<Text name="AlertErrorBody2">Информация введена не полностью. Введите имя</Text>', 'standart', 'danger', 'standart');
    }
}
...
//        document.getElementById("Level2").style.display = "inline-block";
        ...
        fullName.setAttribute("name", "desc");
...
function creatingOptions(array, type) {
    for (let i = 0; i < array.length; i++) {
        ...
        option.setAttribute("name", "disable");
...
function decodeTheTimeCode(value) {
    ...
    try {
        ...
        result = [];
        alerter('<Text name="AlertErrorHeader">Ошибка</Text>', '<Text name="AlertErrorBody3">Неверный формат</Text>', 'standart', 'danger', 'standart');
    }
    return result;
}
...
function createModal() {
    if (!body.contains(document.querySelector(".filter-modal"))) {
        ...
        titleText.setAttribute("name", "FilterTitle");
        ...
        coursesDivTitle.setAttribute("name", "CoursesDivTitle");
        gradesDivTitle.setAttribute("name", "GradesDivTitle");
        ...
        applyFilterEsc.setAttribute("name", "Close");
        ...
        applyFilter.setAttribute("name", "Apply");
...
window.onload = () => {
    ...
    showingCodeButton.addEventListener("click", () => {
        ...
        notifyer("Сформирован Ваш код времени! Можете его отправить пользователю системы",500)
    });
...
courseNameSubmitUpdate.addEventListener("click", () => {
    ...
    if (document.getElementById("courseTextArea").value.length > 3) {
        ...
        alerter('<Text name="AlertAttention">Внимание!</Text>', '<Text name="AlertAttention1">Введите или загрузите из хранилища данные по курсам</Text>', 'standart', 'warning', 'standart');
    }
});
...
gradeNameSubmitUpdate.addEventListener("click", () => {
    ...
    if (document.getElementById("gradeTextArea").value.length > 0) {
        ...
        alerter('<Text name="AlertAttention">Внимание!</Text>', '<Text name="AlertAttention2">Введите или загрузите из хранилища данные по уровням</Text>', 'standart', 'warning', 'standart');
    }
});
//
...
export function alerter(titleWin = " title", bodyWin = "body", footer = "standart", aType = "standart", aWidth = "standart") {
    ...
    else if (aType == "success") labelAlert.style.color = "limeGreen";
    else labelAlert.style.color = "black";

    let modalFooterAlert = document.getElementById("modalFooterAlert");
    modalFooterAlert.innerHTML = `<div><button type="button" name="Close" class="btn btn-secondary btn-sm btnCloseAlert m-2 " style="font-weight: 500;color: white; font-size: 1em">Close</button></div>`;
...
export function confirmer(titleWinContent = " title", bodyWinContent) {
    ...
    let modalFooterAlert = document.getElementById("modalFooterAlert");
    modalFooterAlert.innerHTML = `<button type="button" name="ConfirmerOk" id="ConfirmerOk"  class="btn btn-primary btn-sm m-2 " style="font-weight: 500; font-size: 1em">&nbsp;&nbsp;&nbsp;&nbsp;Oк&nbsp;&nbsp;&nbsp;&nbsp;</button>
...
if ((!localStorage.getItem('users')) && (!localStorage.getItem('business'))) {
    ...
    alerter('Добро пожаловать!', '<p style="text-indent:20px;text-align: justify">Вы первый раз запустили систему составления расписания и работы со временем - <b style="color:blue">НайдиВремя!</b></p> <p style="text-indent:20px;text-align: justify">Для демонстрации работы системы загружены тестовые данные по свободному и занятому времени и сделана настройка. После тестирования эти данные Вы можете удалить, настроить систему для себя и продолжить работу! </p><p style="text-indent:20px;text-align: justify;">Перед началом работы, пожалуйста, ознакомьтесь c <a href="guide.html">"Руководством пользователя"</a>', 'standart', 'success', 'standart')
    ...
    let courses_demo = '["Пробное", "Kodu", "Scratch", "CoSpaces", "MitApp Inventor", "Pencil Code", "Python Start", "Godot", "WebStart", "JavaScript Start", "JavaScripts Front", "JavaScript Game", "PHP", "C# Start", "Unity", "Java Start", "Fusion 3D", "ОГЭ", "ЕГЭ", "Project Team", "Препод."]'
