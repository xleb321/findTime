...
document.getElementById("btnSaveTableSchedule").addEventListener('click', () => {
    alerter('<Text name="GuideInfo">Информация</Text>', '<Text name="GuideInfoBody1"> Сохраняет текущее расписание в памяти устройства.</Text>', 'standart', 'info', 'standart')
});

document.getElementById("btnLoadTableSchedule").addEventListener('click', () => {
    alerter('<Text name="GuideInfo">Информация</Text>', '<Text name="GuideInfoBody2">Загружает расписание из памяти устройства. Если в настройках установлена опция "Показывать загруженное расписание при открытии", то при открытии сразу будет показана сохранённая информация.</Text>', 'standart', 'info', 'standart')
});
...
//});
document.getElementById("helpFull").addEventListener('click', () => {
    alerter('<Text name="GuideInfo">Информация</Text>', '<Text name="GuideInfoBody3">Открывает расписание на весь экран. С него также можно посмотреть совпадения по времени, загрузить и выгрузить из памяти, удалить и добавить информацию по времени. Данное представление можно выбрать в настройках для показа при открытии системы.</Text>', 'standart', 'info', 'standart')
});



document.getElementById("erase-button1").addEventListener('click', () => {
    alerter('<Text name="GuideInfo">Информация</Text>', '<Text name="GuideInfoBody4">Очищает таблицу расписания и снимает все отмеченные флаги в разделах "Свободное время" и "Занятое время"</Text>', 'standart', 'info', 'standart')
});


document.getElementById("create").addEventListener('click', () => {
    alerter('<Text name="GuideInfo">Информация</Text>', '<Text name="GuideInfoBody5">Открывает окно в котором можно добавить данные в разделы "Свободное время" или "Занятое время"</Text>', 'standart', 'info', 'standart')
});


document.querySelector(".filtr").addEventListener('click', () => {
    alerter('<Text name="GuideInfo">Информация</Text>', '<Text name="GuideInfoBody6">Во вкладке "Свободное время" открывает окно в котором можно установить фильтр по уровням данных.</Text>', 'standart', 'info', 'standart')
})
